...
喷嘴归零后的软件移动范围是 X/Y `0–125 mm`、Z `0–160 mm`。受探针
X `+26`、Y `+20` 偏移和 `15 mm` 探测边距共同影响，默认 G29 的实际探针区域为
X `26–110 mm`、Y `20–110 mm`，3 × 3 探测坐标分别为
X `{26, 68, 110}`、Y `{20, 65, 110}`。探针区域与喷嘴软件限位不是同一个范围。
...
XYZ 电机持续上电、不超时释放，防止暂停换料期间丢失位置。这是已确认的
目标行为，也是对旧配置错误的修正。
...
断料传感器计划实际安装，因此新版默认启用检测。断料输入 D19 使用内部上拉，
并定义 LOW 为断料；传感器不接时引脚由内部上拉保持 HIGH，正常情况下不会触发
M600，也不影响普通打印。实机仍应在电机和加热器工作时观察开放接插件是否稳定；
临时不使用传感器时也可以执行 `M412 S0`、`M500` 显式关闭检测。
...
- 当前 Mega2560 构建实际编入的缺失项已逐项补充，包括存储卡初始化失败、通用
  轴回零、M428 限位距离错误、PID 循环数、喷头停靠失败和重新加热完成。
- 未启用模块的消息仍允许回退英文；实机遍历菜单后发现的遗漏再继续补充。
- 加热等待时显示当前 SD 文件名已确认不迁移，SD 打印也使用新版标准加热/冷却提示。
...
| `Marlin/Marlin_main.cpp` | G28、`X1`、M109/M190 文件名、运行时参数 | G28 原生替代；`X1` 和文件名显示确认不迁移 |
