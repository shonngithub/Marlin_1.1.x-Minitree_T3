...
| RUN-01 | D19 单路断料传感器 | D19、内部上拉、LOW 触发、默认启用 | 已移植；计划安装，不接时通常不影响使用 |
...
- 断料传感器计划实际安装。未连接时内部上拉应使 D19 保持 HIGH，不触发 M600；
  仍需实测开放接插件在电机和加热器工作时是否保持稳定。
...
- 获取当前机器保存的真实探针 Z 偏移。
- 实机遍历当前启用功能的中文菜单，记录仍显示英文或缺字的项目并按需补充。
